...
"""Dataset classes"""
class AER_video:
    def __init__(self, modal='none', root='', min_seq_len=0):
        assert modal in ['rgb', 'event','none']
        self.modal=modal
        self.root = osp.join(root, 'AER_video')
        ...
        self.cam_ids=np.unique(track_test_mat['cam_id'])
    ...
    def _process_data(self, names, meta_data, home_dir=None, relabel=False, min_seq_len=0):
        ...
        for tracklet_idx in range(num_tracklets):
            ...
            img_paths = [osp.join(self.root, self.modal, home_dir, img_name[:4], img_name) for img_name in img_names]
